...
import { createOrderApi, loginApi, uploadFileApi } from '../../utils/api-client';
...
test.describe('Uploads — security', () => {
  ...
  test('valid PNG upload succeeds', async ({ request }, testInfo) => {
    ...
    const res = await uploadFileApi(request, auth.token, order.id, 'ref.png', PNG_HEADER, 'image/png');
  ...
  test('exe extension rejected', async ({ request }, testInfo) => {
    ...
    const res = await uploadFileApi(request, auth.token, order.id, 'malware.exe', EXE_MAGIC, 'application/octet-stream');
  ...
  test('png extension with pdf magic bytes rejected', async ({ request }, testInfo) => {
    ...
    const res = await uploadFileApi(request, auth.token, order.id, 'fake.png', PDF_MAGIC, 'image/png');
    expect(res.status()).toBe(400);
  });
