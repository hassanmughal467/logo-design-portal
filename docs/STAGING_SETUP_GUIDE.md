...
**API URL:** `https://staging-api.hawkmerchandising.com` (`environment.staging.ts`)
...
```powershell
curl -s https://staging-api.hawkmerchandising.com/health/live
curl -s https://staging-api.hawkmerchandising.com/health/ready
```

Playwright (optional, requires env):

```powershell
$env:STAGING_API_URL = "https://staging-api.hawkmerchandising.com"
cd Frontend
