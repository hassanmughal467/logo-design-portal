...
public class QuotesController : ControllerBase
{
    ...
    public async Task<IActionResult> Create([FromForm] string quote, [FromForm] IFormFileCollection? files)
    {
        ...
        if (request == null)
        {
            return BadRequest(new { error = "Invalid quote payload." });
        }
    ...
    public async Task<IActionResult> DownloadAttachment(Guid id, string fileName)
    {
        ...
        {
            var role = User.FindFirstValue(ClaimTypes.Role);
            var (content, downloadName, contentType) =
                await _quoteService.DownloadQuoteAttachmentAsync(id, fileName, userId, role);
